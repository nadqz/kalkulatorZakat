...
def zakat_penghasilan():
    ...
    st.write("---")
    st.write("Zakat penghasilan atau zakat profesi adalah zakat yang wajib dibayarkan atas pendapatan yang diperoleh dari pekerjaan atau profesi, seperti gaji. Zakat penghasilan termasuk dalam kategori zakat mal, yang merupakan kewajiban membayar zakat karena memiliki atau menyimpan harta seperti uang dan emas.")
    ...
    st.write("Jumlah Penghasilan Bersih per Bulan: IDR {:,.2f}".format(penghasilan_setelah_hutang))
    ...
    if harga_emas <= 0:
        st.error("Harga Emas harus lebih dari 0")
        return
    
    # Menghitung Besaran Nisab Zakat Penghasilan per Bulan
    nisab_penghasilan = (85 / 12 * harga_emas)
    st.write("Besaran Nishab Zakat Penghasilan per Bulan: IDR {:,.2f}".format(nisab_penghasilan))
    ...
    if penghasilan_setelah_hutang >= nisab_penghasilan:
        ...
        st.write("Jumlah Zakat Penghasilan yang Harus Dibayarkan: IDR {:,.2f}".format(zakat_amount))
    else:
